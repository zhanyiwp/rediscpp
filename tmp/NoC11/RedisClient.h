...
{
public:
	explicit RedisScript(shared_ptr<RedisClient> Client, string &Script);
	~RedisScript();
	template <typename... TS> shared_ptr<RawResult> Eval(const char* s, TS... args);
	template <typename... TS> shared_ptr<RawResult>  EvalSha(const char* s, TS... args);
	int Load();
	shared_ptr<BoolArrayResult> Exists(vector<string> Scripts);
	template <typename... TS> shared_ptr<RawResult>  Run(const char* s, TS... args);
	shared_ptr<StatusResult>  Kill();
	shared_ptr<StatusResult>  Flush();
private:
	string _script;
	shared_ptr<RedisClient> _client;
	string _sha1;
};

template <typename... TS> shared_ptr<RawResult> RedisScript::Eval(const char* s, TS... args)
{
	return _client->RawCommand(s, args...);
}

template <typename... TS> shared_ptr<RawResult> RedisScript::EvalSha(const char* s, TS... args)
{
	return _client->RawCommand(s, args...);
}
