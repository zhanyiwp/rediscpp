...
{
	shared_ptr<IntResult> Result = shared_ptr<IntResult>(new IntResult);
	if (Keys.empty())
	{
...
{
	shared_ptr<StringResult> Result = shared_ptr<StringResult>(new StringResult);
	if (Key.empty())
	{
...
{
	shared_ptr<BoolResult> Result = shared_ptr<BoolResult>(new BoolResult);
	if (Key.empty())
	{
		Result->Set(CMD_OPERATE_FAILED, "Key is empty");
		return Result;
...
{
	shared_ptr<StringArrayResult> Result = shared_ptr<StringArrayResult>(new StringArrayResult);
	if (Pattern.empty())
	{
...
{
	shared_ptr<StatusResult> Result = shared_ptr<StatusResult>(new StatusResult);
...
{
	shared_ptr<FloatResult> Result = shared_ptr<FloatResult>(new FloatResult);
	if (Key.empty())
	{
		Result->Set(CMD_OPERATE_FAILED, "Keys is empty");
		return Result;
...
{
	shared_ptr<StringStringMapResult> Result = shared_ptr<StringStringMapResult>(new StringStringMapResult);
	if (Key.empty())
	{
		Result->Set(CMD_OPERATE_FAILED, "Keys is empty");
		return Result;
...
{
	shared_ptr<StringFloatMapResult> Result = shared_ptr<StringFloatMapResult>(new StringFloatMapResult);
	if (Key.empty())
	{
		Result->Set(CMD_OPERATE_FAILED, "Keys is empty");
		return Result;
...
shared_ptr<IntResult> IRedisClient::ZunionStoreWeights(const string& Dst, vector<string>& Keys, const vector<float>& Weights, const string &Aggregate /*= "SUM"*/)
{
	...
	Cmd.push_back(Dst);
	Cmd.push_back(ToString(Keys.size()));
	for (vector<float>::const_iterator it = Weights.begin(); it != Weights.end(); it++)
	{
		Keys.push_back(ToString(*it));
	}
	Keys.push_back(Aggregate);
	_cmd->vCommand(RT_INTEGER, Cmd, Keys, *Result, &Result->val);
	return Result;
...
shared_ptr<RawResult> IRedisClient::RawCommand(const char* Cmd, ...)
{
	...
	shared_ptr<RawResult> Result = shared_ptr<RawResult>(new RawResult);
	...
	for (int i = 0; i<2; i++)
	{
		...
		if (reply)
		{
			Result->val = shared_ptr<RedisReply>(new RedisReply(reply));
...
#if 0
RedisScript::RedisScript(shared_ptr<RedisClient> Client, string &Script)
:_script(Script)
, _client(Client)
, _sha1("")
{
...
/*
shared_ptr<RawResult> RedisScript::Eval(const char* KeyAndAravs, ...)
{
	redisContext* Ctx = NULL;
	redisReply *reply = NULL;
	shared_ptr<RawResult> Result = shared_ptr<RawResult>(new RawResult);
	...
	for (int i = 0; i<2; i++)
	{
		RedisCtxGuard CtxGuard(&(static_cast<RedisNormalCmd*>(_client->_cmd))->_conn_pool);
		Ctx = CtxGuard.GetCtx();
		if (NULL == Ctx)
		{
			continue;
		...
		va_list args;
		va_start(args, KeyAndAravs);
		reply = static_cast<redisReply *>(redisvCommand(Ctx, KeyAndAravs, args));
		va_end(args);
		if (Ctx && Ctx->err == 1) //超时重试
		{
			CtxGuard.GetConn()->SetStatus(CS_UNCONNECTED);
...
shared_ptr<RawResult>  RedisScript::EvalSha(const char* KeyAndAravs, ...)
{
	if (_sha1.empty())
	{
		Load();
...
int RedisScript::Load()
{
	shared_ptr<StringResult> Result = shared_ptr<StringResult>(new StringResult);
	_client->_cmd->Command(RT_STRING, *Result, &Result->val, "script load%s", _script.c_str());
	if (Result->Success())
	{
		_sha1 = Result->GetVal();
		return 0;
...
shared_ptr<BoolArrayResult> RedisScript::Exists(vector<string> Scripts)
{

}

/*
shared_ptr<StringResult> RedisScript::Run(const char* KeyAndAravs, ...)
{

}
*/

shared_ptr<StatusResult>  Kill()
{

}

shared_ptr<StatusResult>  Flush()
{
